...
sig = np.loadtxt("C:/Users/Fedosov/Documents/projects/mks_return_ICA/raw_data/data2half_0808.txt")#svetodata.txt")#not_bad_data2.txt")#data.txt")#not_bad_data2.txt")
...
new_sig = np.zeros_like(sig, dtype = 'complex')[:,0]
...
b, a = sn.butter(1,[8,12],btype = 'bandpass', fs = 1000)

filtered =sn.lfilter(b,a,filtered)

filtered = sn.hilbert(filtered)


new_sig[good_idx] = filtered

new_sig  = np.roll(new_sig,10)
...
phase = np.angle(filtered,deg = True)
...
raw_emg = mne.io.read_raw_edf('C:/Users/Fedosov/Downloads/Pilot 2/Belocopytov Anton_2021_08_07_18_17_20EMG.edf',preload = True)
...
plt.figure()
plt.scatter(phase_arr,emg_amp)


np.save('phase_and_emg_4.npy', (phase_arr,emg_amp))

container = np.load('phase_and_emg_4.npy')
ph =container[0]
am = container[1]
