...
#ica.pre_whitener_ = np.eye(n_channels)
#ica.unmixing_matrix_ = np.eye(n_channels)
#ica.unmixing_matrix_[0,:]  = 1
#ica.unmixing_matrix_[0,:] = np.array([-0.25,-0.25,1.0,-0.25,-0.25])
...
b_dc, a_dc = sn.butter(1,7.0,btype = 'high',fs = srate)
...
ica_filter_norm = ica_filter/np.linalg.norm(ica_filter)
...
b = sn.firwin(3,50,pass_zero = False, fs = 1000)
...
if r/q > 1000:
    ...
    inv_K_int = (1024.0/kf.K.flatten()).astype('int32')
...
rescale_coef = 20e6*(1<<6) # nedoscaling of envelope o eegboard
...
if (len(combined_array)%2 == 0):
    np.concatenate([combined_array,np.array([777])])
