...
'''
import mne
info = mne.create_info(['1','2','3','4','5'], sfreq = 1000, ch_types='eeg')

raw = mne.io.RawArray(sig[:,:5].T, info)

raw.filter(l_freq = 6, h_freq = 100)

raw.plot(scalings=1e4)'''


nT = len(sig)
Nch = 5

is_zeros = np.zeros([nT],dtype = 'int')
sig_int = sig.astype('int')
for i in range(Nch):
    is_zeros  += sig_int[:,i] == 0
is_zeros = (is_zeros == 5).astype('int')


moments = list()
for i in range(1,nT):
    if (is_zeros[i]-is_zeros[i-1]) == 1:
        moments.append(i)
    
time_moments = np.zeros([nT])
time_moments[moments] = 1.0


labels = sig_int[np.array(moments),-1]
    
moments = np.array(moments)
    

moments_180 = moments[labels == 5][None,:]





kalman_ch = sig_int[:,5].astype('float')


time_range = np.arange(-2000,2000)[:,None]

slice_array = time_range+moments_180



epochs = kalman_ch[slice_array]
