...
sig = np.loadtxt("C:/Users/Fedosov/Documents/projects/mks_return_ICA/data_no_noise_smoothed_arduino_triggers/data.txt")#data.txt")#not_bad_data2.txt")


common_data = np.loadtxt("C:/Users/Fedosov/Documents/projects/mks_return_ICA/data_no_noise_smoothed_arduino_triggers/arduino_data.txt")
...
t_range_for_psd = [270000,330000]# [560000, 568000] for 0410exp
...
#arduino_trigger = (read_signal[2500:]>472.0)*1.0
#arduino_moments = ((arduino_trigger[1:]-arduino_trigger[:-1]) > 0)*1.0

#warning!!!!
arduino_timepoints = []#np.where(arduino_moments)[0]

refract_T = 30
refract_c = 100
for i in range(2960,43000):#range(2050,len(read_signal)):
    if (read_signal[i] > 470.0) and (refract_c > refract_T):
        arduino_timepoints.append(i)
        refract_c = 0
    else:
        refract_c += 1
...
arduino_timepoints = np.array(arduino_timepoints)
#plt.figure()
...
cmplx_gt = source_signal+1j*imag_signal

gt_phase = np.angle(cmplx_gt)

arduino_triggers = np.zeros(len(gt_phase))
arduino_triggers[arduino_timepoints] = 1
plt.figure()
plt.plot(gt_phase)
plt.plot(arduino_triggers)
...
recon_arduino_moments = np.zeros(len(arduino_triggers))

arduino_timepoints_corrected = arduino_timepoints-2500
...
target_phases = target_sig[arduino_timepoints_corrected]*20#-90
...
corr = np.correlate(source_signal, registered[5000:], mode='valid')
...
plt.figure()
plt.plot(source_signal[8189:])
plt.plot(registered[5000:]/200000)
