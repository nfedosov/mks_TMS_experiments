...
%load the data 
data = load('/home/alex/Downloads/Telegram Desktop/realdata.mat');
...
b_fir = fir1(fix(Fs/4), [f0-3 f0+3]/(Fs/2));
...
%   high alpha amp chunks
    ...
    subplot(4,3,9)
    plot(e_(3.8e4:4.2e4));%/max(e_(3.8e4:4.2e4)));
    hold on
    plot(e_fir(3.8e4:4.2e4));%/max(e_fir(3.8e4:4.2e4)));
    hold off
    axis tight
...
%   high alpha amp chunks
    ...
    subplot(4,3,12)
    plot(e_cfir(3.8e4:4.2e4)/max(e_cfir(3.8e4:4.2e4)));
    hold on
    plot(e_fir(3.8e4:4.2e4)/max(e_fir(3.8e4:4.2e4)));
    hold off
    axis tight
